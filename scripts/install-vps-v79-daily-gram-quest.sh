...
BACKUP="${BACKEND}.pre-v79-gram-quest-${STAMP}.bak"

 echo '=== FETCH V79 DAILY GRAM QUEST ==='
git fetch origin main
git show origin/main:scripts/patch-vps-v79-daily-gram-quest.py > /tmp/v79-gram.py
python3 -m py_compile /tmp/v79-gram.py
node --check "$BACKEND"
cp -a "$BACKEND" "$BACKUP"

rollback(){
  echo 'ERROR: V79 install failed; restoring backend.' >&2
  cp -a "$BACKUP" "$BACKEND"
  pm2 restart wiener-api >/dev/null 2>&1 || true
}
trap rollback ERR

WIENER_BACKEND_FILE="$BACKEND" python3 /tmp/v79-gram.py
node --check "$BACKEND"
grep -Fq 'WIENER DAILY GRAM QUEST V79' "$BACKEND"

 echo '=== DATABASE DEFAULTS ==='
...
set gram_quest_enabled=true,
    gram_quest_required_ads=50,
    gram_quest_reward=0.03
where id=true;
...
select gram_quest_enabled,gram_quest_required_ads,gram_quest_reward
from public.app_settings where id=true;
select count(*) as claims_today,coalesce(sum(reward_gram),0) as gram_claimed_today
from public.wiener_gram_daily_quest_claims where quest_day=current_date;
"

echo '=== V79 DAILY GRAM QUEST READY ==='
echo 'Quest: 50 verified MAIN AdsGram ads -> 0.03 GRAM -> once per UTC day.'
echo 'Bonus Ads and Spin Ads do not count.'
