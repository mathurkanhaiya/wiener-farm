...
BACKUP="${BACKEND}.pre-v76-user-ui-${STAMP}.bak"
...
echo '=== V76 USER BOT UI READY ==='
echo "Protected webhook smoke: HTTP $code"
echo 'User menu: /start /wallet /rewards /network /progress /activity /status /profile /help'
echo 'Legacy user commands still work but are hidden from the command menu.'
echo 'Admin handlers/commands, payouts, economy and Mini App were not changed.'
echo 'Run /syncbot once as admin so Telegram refreshes the visible command list.'
