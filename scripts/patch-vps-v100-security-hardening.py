...
p=Path('/opt/wiener-backend/server.mjs');s=p.read_text();anchor="app.use('/functions/v1/wiener-ad',cooldownGuardV67('main','start','complete'));"
...
# Never trust browser interaction as monetary authority.
...
# Invalidate previous unfinished main session before issuing a new one; DB unique index closes races.
needle="      const nonce=crypto.randomBytes(32).toString('hex');"
repl="      await pool.query(`update public.ad_sessions set status='expired' where telegram_id=$1 and network='adsgram' and status in ('started','verified')`,[id]);\n\n      const nonce=crypto.randomBytes(32).toString('hex');"
if needle in s:s=s.replace(needle,repl,1)
# Bonus stacking: only newest session remains claimable.
needle="""      const ins=await pool.query(`
        insert into public.tads_ad_sessions("""
repl="""      await pool.query(`update public.tads_ad_sessions set status='expired' where telegram_id=$1 and widget_id=$2 and status='started'`,[id,BONUS_WIDGET_ID]);
...
s=s.replace(old,new,1);p.write_text(s);print('V100 server patch applied')
