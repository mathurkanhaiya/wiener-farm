...
# V100 request guard: strict actions, UUID validation, server-issued spin tokens.
...
function actionGuardV100(path,allowed,{uuidField=null,allowEmpty=false}={}){
  app.use(path,(req,res,next)=>{
    ...
    if((!a&&!allowEmpty)|| (a&&!allowed.includes(a)))return res.status(400).json({ok:false,error:'unsupported_action',message:'Unsupported action'});
    if(uuidField&&req.body?.[uuidField]&&!UUID_V100.test(String(req.body[uuidField])))return res.status(400).json({ok:false,error:'invalid_request',message:'Invalid request'});
    next();
  });
}
...
actionGuardV100('/functions/v1/wiener-task-api',['claim','begin_external','submission_status','submit_proof'],{uuidField:'task_id'});
...
// Spin authorization is server-issued. Client UUIDs are no longer authorization.
app.post('/functions/v1/wiener-spin',async(req,res,next)=>{
  try{
    ...
    if(a==='spin_start'){
      ...
      await pool.query(`insert into public.wiener_spin_tokens(token,telegram_id,expires_at) values($1,$2,now()+interval '60 seconds')`,[token,id]);
      return res.json({ok:true,data:{spin_token:token,expires_in:60}});
    }
    if(a!=='spin')return next();
    const {id}=await edgeUser(b),token=String(b.spin_token||'');
    if(!/^[0-9a-f]{64}$/i.test(token))return res.status(400).json({ok:false,error:'invalid_spin_token',message:'Spin authorization expired'});
    const take=await pool.query(`update public.wiener_spin_tokens set consumed_at=now() where token=$1 and telegram_id=$2 and consumed_at is null and expires_at>now() returning token`,[token,id]);
    if(!take.rowCount){
      const old=await pool.query(`select 1 from public.wiener_spin_events where idempotency_key=$1 and telegram_id=$2 limit 1`,[token,id]);
      if(!old.rowCount)return res.status(409).json({ok:false,error:'spin_token_used',message:'Spin authorization expired'});
    }
    req.body.idempotency_key=token;
    const originalJson=res.json.bind(res);
    res.json=function(payload){
      if(!payload||payload.ok!==true){pool.query(`update public.wiener_spin_tokens t set consumed_at=null where token=$1 and telegram_id=$2 and not exists(select 1 from public.wiener_spin_events e where e.idempotency_key=$1 and e.telegram_id=$2)`,[token,id]).catch(()=>{});}
      return originalJson(payload);
    };
    return next();
  }catch(e){console.error('v100_spin_guard',String(e?.message||e));return res.status(400).json({ok:false,error:'invalid_request',message:'Invalid request'})}
});
...
# Main ad: client interaction bit is never authoritative. DB function decides from callback verification.
...
# Sponsored task start: expire stale session, reuse a live one, enforce 20/day before insert.
old="""    if(action==='start'){
