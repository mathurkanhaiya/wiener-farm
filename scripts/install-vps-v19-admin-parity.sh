...
"
echo "required_tables=$required_tables/9"
[[ "$required_tables" == "9" ]] || { echo 'ERROR: one or more old admin/bot session tables are missing' >&2; exit 1; }
...
echo '=== LEGACY BOT ROUTE PARITY ==='
SECRET=$(runuser -u postgres -- psql -d "$DB" -Atc "select coalesce(telegram_webhook_secret,'') from public.app_settings where id=true limit 1")
for fn in wiener-bot-admin wiener-bot-user-inspector wiener-bot-broadcast wiener-bot-pay wiener-bot-pay-ton wiener-bot-treasury wiener-bot-addtask wiener-bot-addtask-target; do
  c=$(curl -sS -o /dev/null -w '%{http_code}' -X POST -H 'content-type: application/json' -H "x-wiener-internal-secret: $SECRET" --data '{}' "http://127.0.0.1:3000/functions/v1/$fn" || true)
  echo "$fn -> HTTP $c"
  [[ "$c" != "000" && "$c" != "404" && "$c" != "502" ]] || { unset SECRET; echo "ERROR: $fn unavailable" >&2; exit 1; }
done
unset SECRET
...
admin_action=$(curl -sS -o /dev/null -w '%{http_code}' 'http://127.0.0.1:3000/functions/v1/wiener-admin-action' || true)
echo "wiener-admin-action GET -> HTTP $admin_action"
[[ "$admin_action" != "000" && "$admin_action" != "404" && "$admin_action" != "502" ]] || { echo 'ERROR: one-click admin action route unavailable' >&2; exit 1; }
