...
BACKUP="${BACKEND}.v21-${STAMP}.bak"
cp -a "$BACKEND" "$BACKUP"
rollback(){
  rc=$?
  if [ "$rc" -ne 0 ]; then
    echo 'ERROR: V21 install failed; restoring syntax-valid backend backup.' >&2
    cp -a "$BACKUP" "$BACKEND"
...
echo '=== ENSURE V20B TON CONFIG ==='
bash scripts/install-vps-v20b-ton-config.sh

echo '=== V21 DATABASE RELIABILITY STATE ==='
...
echo '=== INSTALL TON LOCK/BACKOFF/CURSOR + HEALTH ==='
python3 scripts/patch-vps-v21-ton-reliability.py

echo '=== PRE-RESTART SYNTAX + STATIC GATE ==='
node --check "$BACKEND"
...
code=$(curl -sS -o /tmp/v21-webhook.out -w '%{http_code}' -X POST -H 'content-type: application/json' -d '{}' http://127.0.0.1:3000/functions/v1/wiener-bot-webhook || true)
echo "wiener-bot-webhook -> HTTP $code"
[[ "$code" != "000" && "$code" != "404" && "$code" != "502" ]] || { echo 'ERROR: webhook smoke failed' >&2; exit 1; }

echo '=== V21 SAFETY STATE ==='
...
[[ "$auto" == "f" ]] || { echo 'ERROR: V21 refuses to finish with TON autopay enabled' >&2; exit 1; }

echo '=== V21 TON RELIABILITY INSTALLED ==='
echo 'TON scanner: serialized + cursor-based + provider backoff.'
echo 'Manual and cron scans share one lock.'
echo 'Raw provider errors are replaced with safe operator messages.'
