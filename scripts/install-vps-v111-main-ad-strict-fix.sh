...
if start<0 or end<0: raise SystemExit('ERROR: V109 middleware block missing')
end=end+len('// === END WIENER MAIN AD UP TO 20 V109 ===')
# Remove old V109 registration too, then install one authoritative middleware.
s=s[:start]+s[end:]
s=s.replace("app.use('/functions/v1/wiener-ad',mainAdRewardV109());\n",'')
marker='// === WIENER MAIN AD STRICT DYNAMIC V111 ==='
anchor="app.post('/functions/v1/wiener-ad'"
if marker not in s:
 if anchor not in s: raise SystemExit('ERROR: wiener-ad anchor missing')
 code=r'''
  ...
  res.json=function(payload){
   ...
   return (async()=>{
    ...
    try{
     ...
     if(old){await c.query('commit');target.reward=Number(old.final_reward);target.full_reward=Number(old.original_reward);target.configured_reward=Number(old.original_reward);target.base_reward=Number(old.final_reward);target.bonus_reward=Math.max(0,Number(old.original_reward)-Number(old.final_reward));target.bonus_unlocked=!!old.interacted;target.interaction_detected=!!old.interacted;return originalJson(payload)}
     // Existing callback's reward is the admin-configured reward snapshot for this completed session.
     ...
     if(!visibilityQualified){
       const min=configured*0.30,max=configured*0.60;
       // Integer admin rewards use integer partials (10 => 3/4/5/6). Decimal rewards keep precision.
       if(Number.isInteger(configured)){
         const lo=Math.max(0,Math.ceil(min)),hi=Math.max(lo,Math.floor(max));
         finalReward=lo+crypto.randomInt(0,hi-lo+1);
       }else{
         finalReward=Math.round((min+Math.random()*(max-min))*1000)/1000;
       }
     }
