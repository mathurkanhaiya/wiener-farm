...
// The VPS installer patches Ambassador.tsx before `npm run build`. In that
// deployment path the feature is already present, so the prebuild must be a
// no-op instead of looking for the old pre-patch anchors.
if(
  s.includes(MARK) ||
...
// Standalone/frontend-only build fallback. The normal VPS installer uses the
// Python patch first; this path keeps plain `npm run build` compatible too.
one(
