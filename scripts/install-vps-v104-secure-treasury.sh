...
CREATE TABLE IF NOT EXISTS public.treasury_ad_sessions(
 ...
 eligible_at timestamptz, expires_at timestamptz, credited_at timestamptz,
 reward_day date DEFAULT (now() at time zone 'utc')::date
);
-- Upgrade old Treasury table safely instead of assuming CREATE TABLE changed its schema.
...
COMMIT;
SQL
# Install the backend route from a versioned source file; avoids fragile inline migrations.
ROUTE=/opt/wiener-code/scripts/v104-treasury-route.txt
[ -f "$ROUTE" ] || { echo 'Missing v104 route source'; exit 1; }
python3 - "$SERVER" "$ROUTE" <<'PY'
from pathlib import Path
import sys
p=Path(sys.argv[1]); route=Path(sys.argv[2]).read_text(); s=p.read_text(); marker='// V104 SECURE WIENER TREASURY'
if marker not in s:
 pos=max(s.rfind('app.listen('),s.rfind('server.listen(')); s=s[:pos]+route+'\n'+s[pos:] if pos>=0 else s+'\n'+route
 p.write_text(s)
PY
node --check "$SERVER"
npm run build
