...
BACKUP="${BACKEND_FILE}.v98.$(date +%Y%m%d-%H%M%S).bak"
cp -a "$BACKEND_FILE" "$BACKUP"
echo "Backup: $BACKUP"

rollback(){
  echo 'V98 install failed — restoring backend backup'
  cp -a "$BACKUP" "$BACKEND_FILE" || true
  pm2 restart "$PM2_APP" --update-env >/dev/null 2>&1 || true
}
trap rollback ERR

# Schema is created as postgres so the runtime role never needs CREATE permission.
runuser -u postgres -- psql -v ON_ERROR_STOP=1 -d "$DB" <<SQL
CREATE TABLE IF NOT EXISTS public.referral_v2_config(
  id boolean PRIMARY KEY DEFAULT true CHECK(id=true),
  started_at timestamptz NOT NULL DEFAULT now()
);
INSERT INTO public.referral_v2_config(id,started_at)
VALUES(true,now())
...
-- V98 is the only referral payout engine for referrals created after started_at.
-- Disable the legacy flat payout values to prevent double-crediting if those columns exist.
DO \$\$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='app_settings' AND column_name='referral_signup_reward') THEN
    EXECUTE 'UPDATE public.app_settings SET referral_signup_reward=0 WHERE id=true';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='app_settings' AND column_name='referral_active_reward') THEN
    EXECUTE 'UPDATE public.app_settings SET referral_active_reward=0 WHERE id=true';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='app_settings' AND column_name='referral_commission_percent') THEN
    EXECUTE 'UPDATE public.app_settings SET referral_commission_percent=0 WHERE id=true';
  END IF;
END \$\$;
SQL
...
# Verify the new schema is visible to the app role.
...
trap - ERR
echo '=== V98 REFERRAL COUNTRY REWARDS READY ==='
echo 'Rewards: Tier1 600/10 ads | Tier2 500/10 | Tier3 400/5 | Tier4 300/5'
echo 'Join stage: 100 WIENER. VPN/proxy pauses reward. Multi-account/self-referral bans the referred account.'
if grep -q '^IPAPI_IS_KEY=.' "$BACKEND_DIR/.env" 2>/dev/null; then
  echo 'VPN detection: configured'
else
  echo 'VPN detection: NOT CONFIGURED — set IPAPI_IS_KEY to enable VPN/proxy detection'
fi
