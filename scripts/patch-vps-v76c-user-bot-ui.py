...
replacement=r'''sync_anchor="async function internalV18("
if sync_anchor not in s:
    raise SystemExit('ERROR: sync wrapper anchor missing')
override=r'''
// === WIENER USER COMMAND SYNC OVERRIDE V76C ===
const syncTelegramV18Base76C=syncTelegramV18;
syncTelegramV18=async function(){
  const info=await syncTelegramV18Base76C();
...
// === END WIENER USER COMMAND SYNC OVERRIDE V76C ===
'''
