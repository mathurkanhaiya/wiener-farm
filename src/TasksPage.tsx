...
async function botTask(action:'begin'|'status',taskId:string){
  const r=await fetch(`${SUPABASE_URL}/functions/v1/wiener-bot-task`,{method:'POST',headers:{'Content-Type':'application/json','apikey':PUBLISHABLE_KEY},body:JSON.stringify({action,task_id:taskId,initData:getInitData()})});
  const x=await r.json().catch(()=>({ok:false,error:'invalid_response'}));if(!r.ok||!x.ok)throw new Error(x.message||x.error||'Verification failed');return x.data;
}
async function taskApi(taskId:string,action='claim'){
  const r=await fetch(`${SUPABASE_URL}/functions/v1/wiener-task-api`,{method:'POST',headers:{'Content-Type':'application/json','apikey':PUBLISHABLE_KEY},body:JSON.stringify({action,task_id:taskId,initData:getInitData()})});
  const x=await r.json().catch(()=>({ok:false,error:'invalid_response'}));if(!r.ok||!x.ok)throw new Error(x.message||x.error||'Task verification failed');return x.data;
}
...
function AdsGramTaskBlock({say}:{say:(s:string)=>void}){
  ...
  useEffect(()=>{
    let active=true,rewarded=false,retry:any=null,readyTimer:any=null;const root=mount.current;if(!root)return;root.innerHTML='';setState('loading');
    (async()=>{try{
      const session=await adsgramTaskApi('start');if(!active)return;
      if(!customElements.get('adsgram-task'))throw new Error('AdsGram task SDK unavailable');
      ...
      const onReward=async()=>{if(rewarded||!active)return;rewarded=true;setState('crediting');try{const x=await adsgramTaskApi('reward',{session_id:session.session_id});if(!active)return;say(`+${Number(x?.reward||2)} WIENER`);setState('waiting');window.setTimeout(()=>active&&setCycle(v=>v+1),350)}catch(e:any){rewarded=false;setState('error');say(e?.message||'Task reward failed')}};
      ...
      el.addEventListener('reward',onReward as EventListener);el.addEventListener('onBannerNotFound',onMissing as EventListener);el.addEventListener('onError',onError as EventListener);root.appendChild(el);
      readyTimer=window.setTimeout(()=>{if(active&&!rewarded)setState('ready')},350);
      return()=>{el.removeEventListener('reward',onReward as EventListener);el.removeEventListener('onBannerNotFound',onMissing as EventListener);el.removeEventListener('onError',onError as EventListener)};
    }catch(e:any){if(active){setState('error');retry=window.setTimeout(()=>active&&setCycle(v=>v+1),5000)}}})();
    return()=>{active=false;if(retry)clearTimeout(retry);if(readyTimer)clearTimeout(readyTimer);if(root)root.innerHTML=''};
  },[cycle]);
  return <div className={`adsgram-task-shell ${state}`}>
    <style>{`
      .adsgram-task-shell{position:relative;border-bottom:1px solid rgba(255,255,255,.08);min-height:82px}.adsgram-task-shell:last-child{border-bottom:0}.adsgram-task-mount{min-height:82px}.adsgram-native-task{--adsgram-task-font-size:14px;--adsgram-task-icon-size:46px;--adsgram-task-icon-title-gap:14px;--adsgram-task-button-width:76px;--adsgram-task-icon-border-radius:15px;display:block;width:100%;padding:18px 0;background:transparent;color:#fff;font-family:inherit}.adsgram-task-reward{display:block;margin-top:4px;color:#a3b5a6;font-size:13px;font-weight:800}.adsgram-task-button{display:inline-flex;align-items:center;justify-content:center;min-width:76px;min-height:40px;padding:0 14px;border-radius:14px;background:linear-gradient(180deg,#fff05f,#ffd20b 63%,#edb900);color:#272000;font-size:12px;font-weight:950;letter-spacing:.5px}.adsgram-task-button.done{filter:saturate(.45);opacity:.7}.adsgram-task-status{position:absolute;inset:0;display:none;align-items:center;gap:14px;padding:18px 0;background:linear-gradient(155deg,rgba(25,92,55,.98),rgba(15,73,43,.98));z-index:2}.adsgram-task-shell.crediting .adsgram-task-status,.adsgram-task-shell.waiting .adsgram-task-status,.adsgram-task-shell.unavailable .adsgram-task-status,.adsgram-task-shell.error .adsgram-task-status,.adsgram-task-shell.loading .adsgram-task-status{display:flex}.adsgram-task-status .square{width:50px;height:50px;border-radius:16px}.adsgram-task-status b{display:block;font-size:15px}.adsgram-task-status small{display:block;margin-top:3px;color:#a3b5a6;font-size:12px;font-weight:700}.adsgram-task-dot{width:8px;height:8px;border-radius:50%;background:#ffe025;box-shadow:0 0 0 5px rgba(255,224,37,.12)}
    `}</style>
    <div className="adsgram-task-mount" ref={mount}/>
    <div className="adsgram-task-status"><div className="square check"><AnimatedIcon name="ads" active={state==='ready'}/></div><div className="grow"><b>{state==='crediting'?'Claiming sponsored reward':state==='waiting'?'Task completed':state==='unavailable'?'No sponsored task right now':state==='error'?'Refreshing sponsored task':'Loading sponsored task'}</b><small>{state==='waiting'?'Locked until the next AdsGram task appears.':state==='unavailable'||state==='error'?'Checking again automatically…':'Please wait…'}</small></div><span className="adsgram-task-dot"/></div>
  </div>;
}
