...
export function Tasks({data,run,say}:{data:Snapshot;run:any;say:(s:string)=>void}){
  ...
  const normalTask=async(t:any)=>{
    ...
    try{
      if(!opened){
        ...
        say(t.verification==='telegram_member'?'Join the channel/group, then return and tap CHECK':'Open the task, then tap CHECK');
        return;
      }
      setBusy(t.id);
    ...
    }catch(e:any){say(e.message||'Task verification failed')}finally{setBusy('')}
  };
  ...
  return <>
    ...
    <section className="card task-list">{items.length?items.map(t=>{
      ...
      const label=done.has(t.id)?'DONE':busy===t.id?'WAIT':isBot?(state==='verified'?'CLAIM':state==='pending'?'CHECK':'START BOT'):(normalOpened?'CHECK':'JOIN');
      const progress=t.max_completions?`${Number(t.completed_count||0)}/${Number(t.max_completions)} completed · `:'';
