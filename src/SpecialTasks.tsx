...
export function SpecialTasks({data,say,refresh}:{data:Snapshot;say:(s:string)=>void;refresh:()=>Promise<any>}){
 const {t}=useI18n();
 const tasks=useMemo(()=>data.tasks.filter((task:any)=>task.category==='special'&&task.enabled!==false).sort((a:any,b:any)=>Number(a.sort_order||0)-Number(b.sort_order||0)),[data.tasks]);
 const done=new Set(data.completed.map((x:any)=>x.task_id));
 const [busy,setBusy]=useState('');
 if(!tasks.length)return null;
 const verify=async(task:any)=>{if(busy||done.has(task.id))return;try{setBusy(task.id);if(task.verification==='profile_name'||task.verification==='profile_bio')await specialTaskApi(task.id,'check');await specialTaskApi(task.id,'claim');say(`${task.verification==='none'?'✅ Completed':'✅ Verified'} · +${task.reward} W`);await refresh()}catch(e:any){const m=String(e?.message||'Verification failed');const pretty:Record<string,string>={profile_name_requirement_not_met:'Add WIENER or 🌭 to your Telegram name, then check again.',profile_bio_requirement_not_met:'Add the required Wiener Farm link/text to your Telegram bio, then check again.',open_bot_chat_first:'Open Wiener Farm bot once, then try CHECK again.'};say(pretty[m]||m)}finally{setBusy('')}};
 const label=(task:any)=>done.has(task.id)?t('common.done','DONE'):busy===task.id?'…':task.verification==='none'?t('common.claim','CLAIM'):'CHECK';
  ...
  `}</style><section className="special-wrap"><div className="special-head"><div><span>{t('special.title','SPECIAL MISSIONS')}</span><h3>{t('special.subtitle','Extra ways to earn')}</h3></div><small>{tasks.filter((task:any)=>done.has(task.id)).length}/{tasks.length} {t('common.done','done')}</small></div><div className="card special-list">{tasks.map((task:any)=>{const auto=task.verification==='profile_name'||task.verification==='profile_bio';return <div className="special-row" key={task.id}><div className="square check"><AnimatedIcon name={auto?'check':'gift'} active={!done.has(task.id)}/></div><div className="special-copy"><h4>{task.title}</h4><p>{task.description||'Complete this mission and claim your reward.'}</p><div className="special-meta"><span className="special-chip reward">+{task.reward} W</span><span className="special-chip">{task.is_daily?t('tasks.daily','DAILY'):'ONE-TIME'}</span><span className="special-chip">{auto?'AUTO CHECK':'DIRECT CLAIM'}</span></div></div><button className="primary small" disabled={done.has(task.id)||busy===task.id} onClick={()=>verify(task)}>{label(task)}</button></div>})}</div></section></>;
}
