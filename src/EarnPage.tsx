...
type OffersStatus={
  providers?:{lootably?:{ready:boolean;name:string};bitlabs?:{ready:boolean;name:string}};
...
export function EarnPage(){
  ...
  return <section style={{paddingBottom:18}}>
    ...
    <div className="card" style={{marginBottom:12}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center'}}><div><div className="eyebrow" style={{textAlign:'left'}}>{section==='offerwall'?'LOOTABLY':'BITLABS'}</div><h3 style={{margin:'4px 0'}}>{section==='offerwall'?'Games, apps & offers':'Paid surveys'}</h3></div><span style={{fontSize:10,fontWeight:900,padding:'6px 8px',borderRadius:999,background:ready?'rgba(50,220,120,.11)':'rgba(255,190,55,.10)',color:ready?'#72f2a8':'#ffd46b'}}>{loading?'CHECKING':ready?'READY':'SETUP'}</span></div>
      <p style={{fontSize:12,lineHeight:1.6,opacity:.62}}>{section==='offerwall'?'Complete games, installs, sign-ups and partner offers selected for your device and region.':'Take surveys matched by BitLabs. Qualification and final reward depend on provider verification.'}</p>
      <div style={{padding:12,borderRadius:14,background:'rgba(255,255,255,.035)',fontSize:11,lineHeight:1.55,opacity:.72,marginBottom:12}}>{section==='offerwall'?'Complete offer → Lootably verifies → WIENER credited':'Complete survey → BitLabs verifies → WIENER credited'}</div>
      <button className="primary" disabled={loading||!!opening||!ready} onClick={()=>openProvider(provider)}>{opening===provider?'OPENING…':ready?(section==='offerwall'?'OPEN OFFERWALL':'START SURVEYS'):'PROVIDER SETUP REQUIRED'}</button>
    </div>
