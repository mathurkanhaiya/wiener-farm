...
export function EarnPage(){
  ...
  useEffect(()=>{refresh()},[]);
  const openProvider=async()=>{const provider='offerwallgg';if(opening)return;try{setOpening(provider);setError('');const x=await offersApi('open',{provider});const url=String(x?.url||'');if(!/^https:\/\//i.test(url))throw new Error('Provider link unavailable');try{(window.Telegram?.WebApp as any)?.HapticFeedback?.impactOccurred?.('light')}catch{}setFrameLoading(true);setOfferUrl(url)}catch(e:any){setError(String(e?.message||e))}finally{setOpening('')}};
  const closeProvider=()=>{setOfferUrl('');setFrameLoading(false);refresh()};
  const ready=status?.providers?.offerwallgg?.ready;
  const history=(status?.history||[]).filter(x=>x.provider==='offerwallgg').slice(0,6);

  if(offerUrl)return <section style={{paddingBottom:18}}>
    <div className="card" style={{marginBottom:10,padding:10,display:'flex',alignItems:'center',justifyContent:'space-between',gap:10}}>
      <button className="secondary" style={{width:'auto',padding:'9px 12px',fontSize:11}} onClick={closeProvider}>← BACK</button>
      <b style={{fontSize:12}}>Offerwall.GG</b>
      <button className="secondary" style={{width:'auto',padding:'9px 12px',fontSize:11}} onClick={()=>{const u=offerUrl;setOfferUrl('');setFrameLoading(true);setTimeout(()=>setOfferUrl(u),0)}}>RELOAD</button>
    </div>
    <div style={{position:'relative',width:'100%',height:'calc(100vh - 155px)',minHeight:560,borderRadius:18,overflow:'hidden',background:'rgba(255,255,255,.035)',border:'1px solid rgba(255,255,255,.08)'}}>
      {frameLoading&&<div style={{position:'absolute',inset:0,display:'grid',placeItems:'center',fontSize:12,fontWeight:800,opacity:.65,zIndex:0}}>LOADING OFFERWALL.GG…</div>}
      <iframe
        title="Offerwall.GG"
        src={offerUrl}
        ...
        style={{position:'relative',zIndex:1,width:'100%',height:'100%',border:0,background:'transparent'}}
      />
    </div>
  </section>;
