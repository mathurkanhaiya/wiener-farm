...
export function V2MissionBoard({state,setState,say}:Props){
  const steps=[
    {name:'Claim Faucet',ok:state.faucetClaims>0},
    {name:'Complete 1 Micro Task',ok:state.done.length>0},
    {name:'Earn first Ticket',ok:state.tickets>0},
    {name:'Reach 20 XP',ok:state.xp>=20},
    {name:'Enter test Giveaway',ok:state.entries>0},
  ];
...
export function V2TicketDraw({state,setState,say}:Props){
  const enter=()=>{if(state.tickets<1){say('Earn a test ticket first.');return}setState(s=>({...s,tickets:s.tickets-1,entries:s.entries+1}));say('Test entry added. No real giveaway entry was created.')};
  return <section className="v2-draw-card"><div><span>🎁 TEST COMMUNITY GIVEAWAY</span><h3>3 winners · sandbox draw</h3><p>Entry costs 1 test ticket. Tickets have no cash value and cannot be withdrawn.</p></div><div className="v2-draw-stats"><b>{state.tickets} 🎟</b><span>{state.entries} entries</span></div><button onClick={enter}>ENTER WITH 1 TEST TICKET</button></section>
}
