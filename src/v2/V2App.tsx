...
import {V2Faucet,V2MicroTasks,V2Offerwall,V2SandboxWallet,V2MissionBoard,V2TicketDraw,useV2Sandbox} from './V2Sandbox';
import {V2EconomyCenter} from './V2Economy';
import './v2.css';

type V2Tab='home'|'earn'|'rewards'|'compete'|'me';
type EarnView='overview'|'ads'|'tasks'|'daily'|'claim'|'faucet'|'micro'|'offerwall';
type RewardView='overview'|'daily'|'claim'|'mission'|'draw';
type CompeteView='overview'|'leaderboard';
type MeView='overview'|'profile'|'wallet'|'ambassador'|'economy';
...
function Home({data,setTab,setEarn,setRewards,setCompete,setMe,journey}:{data:Snapshot;setTab:(x:V2Tab)=>void;setEarn:(x:EarnView)=>void;setRewards:(x:RewardView)=>void;setCompete:(x:CompeteView)=>void;setMe:(x:MeView)=>void;journey:{done:number;total:number;percent:number}}){
 ...
 return <div className="v2-page">
  ...
  <div className="v2-quick-row"><button onClick={()=>{setEarn('faucet');setTab('earn')}}><span>💧</span><b>Faucet</b></button><button onClick={()=>{setEarn('micro');setTab('earn')}}><span>🧩</span><b>Micro</b></button><button onClick={()=>{setEarn('offerwall');setTab('earn')}}><span>🎮</span><b>Offers</b></button><button onClick={()=>{setRewards('mission');setTab('rewards')}}><span>🔥</span><b>Journey</b></button></div>
  <section className="v2-next-card"><div className="v2-next-top"><div><span>STARTER JOURNEY · TEST</span><h3>Learn V2 by earning in the sandbox</h3></div><b>{journey.percent}%</b></div><div className="v2-progress"><i style={{width:`${journey.percent}%`}}/></div><div className="v2-next-meta"><span>{journey.done}/{journey.total} steps</span><span>{ads} ads today</span><span>{tasks} tasks completed</span></div><button onClick={()=>{setRewards('mission');setTab('rewards')}}>CONTINUE JOURNEY</button></section>
  <SectionHead eyebrow="FOR YOU" title="What matters now" sub="A cleaner home that prioritizes the next useful action."/>
  <div className="v2-stack"><ActionCard icon="⚡" title="Earn Hub" text="Faucet, ads, tasks, micro tasks, farm and offers." reward="OPEN" onClick={()=>setTab('earn')} badge="V2"/><ActionCard icon="🎁" title="Rewards Hub" text="Tickets, missions, giveaways and drops." reward="EXPLORE" onClick={()=>setTab('rewards')}/><ActionCard icon="🏆" title="Weekly competition" text={rank?`Current rank #${rank}`:'Earn activity points to enter rankings.'} reward="VIEW" onClick={()=>{setCompete('leaderboard');setTab('compete')}}/><ActionCard icon="🧮" title="V2 Economy Center" text="Funding pools, budgets, liability and safety rules." reward="ADMIN" onClick={()=>{setMe('economy');setTab('me')}} badge="TEST"/></div>
 </div>
}
...
function EarnHub({view,setView,data,refresh,say,run,runFarm,sandbox}:{view:EarnView;setView:(x:EarnView)=>void;data:Snapshot;refresh:()=>Promise<any>;say:(s:string)=>void;run:Props['run'];runFarm:Props['runFarm'];sandbox:ReturnType<typeof useV2Sandbox>}){
 ...
 return <div className="v2-page"><SectionHead eyebrow="EARN HUB" title="More than watch & earn" sub="Quick rewards, real tasks and higher-value opportunities in one place."/><V2SandboxWallet state={sandbox.state}/><div className="v2-feature-grid"><button className="v2-feature-card primary" onClick={()=>setView('faucet')}><span>💧</span><small>RETENTION</small><h3>Faucet</h3><p>5 WIENER test claim · planned 4h cooldown · 4/day.</p><b>TEST NOW</b></button><button className="v2-feature-card" onClick={()=>setView('micro')}><span>🧩</span><small>SPONSOR / PLATFORM</small><h3>Micro Tasks</h3><p>Verified actions with clear funding and completion caps.</p><b>OPEN LAB</b></button><button className="v2-feature-card" onClick={()=>setView('offerwall')}><span>🎮</span><small>PROVIDER FUNDED</small><h3>Offerwall</h3><p>Games, apps, surveys and partner conversions.</p><b>EXPLORE</b></button><button className="v2-feature-card" onClick={()=>setView('ads')}><span>📺</span><small>LIVE</small><h3>Rewarded Ads</h3><p>Current working ad flow inside V2.</p><b>OPEN</b></button><button className="v2-feature-card" onClick={()=>setView('tasks')}><span>✅</span><small>LIVE</small><h3>Task Center</h3><p>Sponsored, Telegram and community tasks.</p><b>EXPLORE</b></button><button className="v2-feature-card" onClick={()=>setView('claim')}><span>🌾</span><small>LIVE</small><h3>Farm</h3><p>Current farming claim flow.</p><b>CLAIM</b></button></div><section className="v2-earn-map"><span>V2 EARNING MAP</span><div><i>💧 Faucet</i><b>→</b><i>🧩 Micro</i><b>→</b><i>✅ Tasks</i><b>→</b><i>🎮 Offers</i></div><p>Every monetary reward will carry a funding source before it can become live.</p></section></div>
}

function RewardsHub({view,setView,data,run,runFarm,sandbox,say}:{view:RewardView;setView:(x:RewardView)=>void;data:Snapshot;run:Props['run'];runFarm:Props['runFarm'];sandbox:ReturnType<typeof useV2Sandbox>;say:(s:string)=>void}){
 ...
 if(view==='mission')return <div className="v2-page"><V2Back title="Starter Journey" onBack={()=>setView('overview')}/><V2SandboxWallet state={sandbox.state}/><V2MissionBoard state={sandbox.state} setState={sandbox.setState} say={say}/></div>;
 if(view==='draw')return <div className="v2-page"><V2Back title="Giveaway Test" onBack={()=>setView('overview')}/><V2SandboxWallet state={sandbox.state}/><V2TicketDraw state={sandbox.state} setState={sandbox.setState} say={say}/></div>;
 return <div className="v2-page"><SectionHead eyebrow="REWARDS" title="Come back for something" sub="Tickets, journeys, giveaways and event rewards."/><div className="v2-ticket-hero"><div><span>TEST TICKETS</span><h2>{sandbox.state.tickets} <small>available</small></h2><p>Tickets have no cash value and remain session-only in admin testing.</p></div><div className="v2-ticket">🎟</div></div><div className="v2-stack"><ActionCard icon="🔥" title="Starter Journey" text="Complete varied V2 actions and test the mission reward flow." reward="SANDBOX" onClick={()=>setView('mission')} badge="NEW"/><ActionCard icon="🎉" title="Ticket Giveaway" text="Spend test tickets to simulate a giveaway entry." reward="1 TICKET" onClick={()=>setView('draw')}/><ActionCard icon="🔥" title="Current Daily Reward" text="Existing live daily reward flow." reward="LIVE" onClick={()=>setView('daily')}/><ActionCard icon="🌾" title="Farm Claim" text="Existing live farm claim flow." reward="LIVE" onClick={()=>setView('claim')}/></div><section className="v2-preview-board"><span>V2 REWARD CENTER</span><div><article><b>🎟 Tickets</b><p>Utility only: earned and spent with a future server ledger.</p></article><article><b>⭐ XP & Levels</b><p>Progression only; no direct withdrawal value.</p></article><article><b>🏅 Achievements</b><p>Badges, milestones and non-cash unlocks.</p></article><article><b>🎁 Promo Drops</b><p>Campaign-funded limited reward drops.</p></article></div></section></div>
}
...
function CompeteHub({view,setView,data}:{view:CompeteView;setView:(x:CompeteView)=>void;data:Snapshot}){
 ...
 return <div className="v2-page"><SectionHead eyebrow="COMPETE" title="Climb, defend, win" sub="Every contest in one competitive hub."/><section className="v2-rank-card"><span>CURRENT RANK</span><h2>{data.rank?`#${data.rank}`:'UNRANKED'}</h2><p>Global activity leaderboard</p><button onClick={()=>setView('leaderboard')}>VIEW LEADERBOARD</button></section><div className="v2-stack"><ActionCard icon="📺" title="Weekly Ad League" text="Current valid-ad competition." reward="LIVE" onClick={()=>setView('leaderboard')} badge="LIVE"/><ActionCard icon="👥" title="Referral Race" text="Future competition funded from acquisition / marketing budget." reward="PLANNED" onClick={()=>{}}/><ActionCard icon="✅" title="Task Sprint" text="Future ranking by verified task completions." reward="PLANNED" onClick={()=>{}}/><ActionCard icon="🌍" title="Community Goal" text="Shared milestone unlocks a predefined community pool." reward="PLANNED" onClick={()=>{}}/></div></div>
}
...
export function V2App({data,refresh,say,run,runFarm,onExit}:Props){
 ...
 const section=useMemo(()=>{
  ...
  if(tab==='rewards')return <RewardsHub view={rewardView} setView={setRewardView} data={data} run={run} runFarm={runFarm} sandbox={sandbox} say={say}/>;
