...
function App(){
  ...
  return <div className="app-shell"><Brand data={data}/><main className="content">{tab==='home'&&<HomeWithPromo data={data} run={runWithFarmModal} setTab={setTab} refresh={refresh} say={say}/>} {tab==='ads'&&<Ads data={data} refresh={refresh} say={say}/>} {tab==='tasks'&&<Tasks data={data} run={run} say={say} refresh={refresh}/>} {tab==='invite'&&<Invite data={data} say={say}/>} {tab==='wallet'&&<WalletV2 data={data} setTab={setTab}/>} {tab==='daily'&&<DailyPage data={data} run={run}/>} {tab==='claim'&&<ClaimPage data={data} run={runWithFarmModal}/>} {tab==='leaderboard'&&<LeaderboardPage data={data}/>} {tab==='profile'&&<ProfilePage data={data} setTab={setTab}/>} {tab==='admin'&&data.is_admin&&<><MandatoryAdmin say={say}/><AdminWithdrawUpgrade say={say}/><Admin say={say}/></>}</main><Nav tab={tab} setTab={setTab} admin={data.is_admin}/><FarmClaimModal open={farmClaimOpen} data={data} onClose={()=>setFarmClaimOpen(false)} refresh={refresh} say={say}/>{toast&&<div className="toast">{toast}</div>}</div>
}
export default App;
