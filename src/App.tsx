...
function App(){
  const [tab,setTab]=useState<Tab>(()=>pageFromUrl()),[data,setData]=useState<Snapshot|null>(null),[loading,setLoading]=useState(true),[error,setError]=useState(''),[toast,setToast]=useState(''),[mandatory,setMandatory]=useState<any>(null),[multiBlocked,setMultiBlocked]=useState(false);
  ...
  const runWithFarmAd=async(action:string,b:any={},ok='Done')=>{
    ...
    try{
      ...
      const r=await api('farm_claim',b);
      say(`+${Number(r?.reward||data.settings.farm_claim_reward||0)} WIENER claimed`);
      await refresh();
    }catch(e:any){say(String(e?.message||'Complete the ad and try again.'))}
  };
