...
type GuestTab='home'|'earn'|'tasks'|'profile';
...
export default function GuestApp(){
  ...
  return <div className="app-shell guest-app">
    ...
    <nav style={{position:'fixed',left:12,right:12,bottom:12,zIndex:50,display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:6,padding:7,borderRadius:22,background:'rgba(10,20,16,.88)',border:'1px solid rgba(255,255,255,.1)',backdropFilter:'blur(20px)'}}>
      {([['home','🏠','Home'],['earn','💰','Earn'],['tasks','✅','Tasks'],['profile','👤','Profile']] as const).map(([id,icon,label])=><button key={id} onClick={()=>setTab(id)} style={{border:0,borderRadius:15,padding:'9px 4px',background:tab===id?'rgba(255,255,255,.1)':'transparent',color:'inherit',fontWeight:800}}><span style={{display:'block',fontSize:18}}>{icon}</span><span style={{fontSize:10}}>{label}</span></button>)}
    </nav>
  </div>
}
