  ...
  useEffect(()=>{
    ...
    const sync=()=>{
      ...
      document.querySelectorAll('.amb-info-grid').forEach((grid)=>{
        ...
        if(cards[2]&&cards[2].textContent?.trim()!=='10WIENER/code')cards[2].innerHTML='<b>10</b><span>WIENER/code</span>';
        if(cards[3]&&cards[3].textContent?.trim()!=='24hauto expiry')cards[3].innerHTML='<b>24h</b><span>auto expiry</span>';
      });
      document.querySelectorAll('.amb-info .tiny.center').forEach((el)=>{
        const text='Each successful code claim counts separately for Ambassador commission and leaderboard.';
        if(el.textContent!==text)el.textContent=text;
      });
      document.querySelectorAll('.amb-message-preview').forEach((el)=>{
        const html='<b>👤 First 100 Active Users Only!</b><br>🎁 Reward: 10 WIENER per code<br><br>🎟 Claim Code: <strong>UNIQUE_CODE</strong><br><br>⏳ Expires in 24 hours<br>🔥 Redeem your code and claim your FREE WIENER before all rewards are claimed!';
